...
train_iter, val_iter, vocab = get_iterators(opt, path='./data/', fname=candidates_path,glove_path='glove/glove.6B.100d.txt')
opt.n_vocab = len(vocab['embedding_matrix'])
if opt.training:
    vae = VAE(opt)
    print(vocab.keys())
    vae.embedding.weight.data.copy_(vocab['embedding_matrix'])              #Intialize trainable embeddings with pretrained glove vectors
    vae = get_cuda(vae)
...
def training():
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        for batch in train_iter:
            x = get_cuda(batch) 	                                #Used as encoder input as well as target output for generator
