...
def initialize_train_test_dataset(dataset):
    ...
    if dataset in ['reddit', 'gab']:
        ...
        hate_num = len(X_text)
    elif dataset == 'intentconanv2':
        import pandas as pd
        dataset_fname = './IntentConanv2/train.csv'
        df = pd.read_csv(dataset_fname)
        X_text = [str(row['hatespeech']).strip() for _, row in df.iterrows()]
        Y_text = [[str(row['counterspeech']).strip()] for _, row in df.iterrows()]
        hate_num = len(X_text)
...
def to_method_object(method_name):
    ...
    if method_name == 'TF_IDF':
        ...
        print("ConveRT model unavailable, using USE instead")
        return vector_based.VectorSimilarityMethod(encoder=vector_based.TfHubEncoder("https://tfhub.dev/google/universal-sentence-encoder/4"))
    elif method_name == 'CONVERT_MAP':
        print("ConveRT model unavailable, using USE instead")
        return vector_based.VectorMappingMethod(encoder=vector_based.TfHubEncoder("https://tfhub.dev/google/universal-sentence-encoder/4"))
